    ...
    , ui(new Ui::MainWindow)
{
	...
	gWin = new QMainWindow(this);
	proxyWidget = new QWidget(gWin);
	gWin->setCentralWidget(proxyWidget);
	layout = new QGridLayout(proxyWidget);
	customPlot = new QCustomPlot(proxyWidget);
	graph = new QCPGraph(customPlot->xAxis, customPlot->yAxis);

	gWin->resize(1000,500);
	layout->addWidget(customPlot,0,0,1,1);
	customPlot->setMinimumHeight(200);
	customPlot->setMinimumWidth(400);
	customPlot->setSizePolicy(QSizePolicy::MinimumExpanding, QSizePolicy::MinimumExpanding);

	//QCustomPlot поддержка встроенных функции масштабирования.
...
void MainWindow::on_pb_start_clicked()
{
	...
	auto findMax = [&](QVector<double> res)
	{
		...
		if(graph->data()->isEmpty() == false)
		{
			graph->data()->clear();
		}


		int numPoints = static_cast<int>(kSampling * kGraphTime/1000);
		for(int i = 0; i <= numPoints; ++i)
		{
			double x = static_cast<double>(i)/kSampling * 1000;//шкала X в миллисекундах
			graph->addData(x, res[i]);
		}


		DisplayResult(mins, maxs);
		emit sig_end_calculation();
...
{
	QString wName = "График канала ";
	wName += ui->cmB_numCh->currentText();
	gWin->setWindowTitle(wName);
